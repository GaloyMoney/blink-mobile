...
export const VerticalDataComponent: React.FC<Props> = ({ product, onItemPress }) => {
  return (
    <TouchableOpacity style={styles.container}
      onPress={onItemPress}
    >
      <Image style={{ height: ITEM_WIDTH, width: ITEM_WIDTH }}
        source={images.tempStore} />
      <View style={{ padding: 10 }}>
      <Text style={{fontFamily:typography.bold,fontSize:fontSize.font14}}>$ 200</Text>
      <Text style={{fontFamily:typography.regular,fontSize:fontSize.font16,lineHeight:21}}>{product.name}</Text>
      <Text style={{fontFamily:typography.medium,fontSize:fontSize.font13,color:'#9499A5'}} numberOfLines={2}>{product.description}</Text>
      </View>
    </TouchableOpacity>
  )
}
...
const styles = StyleSheet.create({
  container: { width: ITEM_WIDTH, backgroundColor: '#F4F8FD', borderRadius: 8, },
})
