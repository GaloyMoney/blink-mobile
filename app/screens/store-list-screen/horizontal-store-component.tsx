...
type Props = {
  product: StoreAttributes
  onLocationPress: () => void
  onItemPress: () => void
}

export const LandscapeDataComponent: React.FC<Props> = ({ product, onLocationPress,onItemPress }) => {
  return (
    <TouchableOpacity style={styles.container}
    onPress={onItemPress}
    >
      <Row containerStyle={{ flex: 1 }}>
        <Image style={{ width: 89, height: '100%', borderRadius: 4, marginRight: 10 }}
          source={images.placeholderImage} />
        <View style={{ flex: 1 }}>
          <Text style={styles.bigText}>{product.name}</Text>
          <Row containerStyle={{ alignItems: 'center' }}>
            <FilledDirectionSvg />
            <Text style={styles.smallText}>{' '}{product.distance}m</Text>
          </Row>
...
const styles = StyleSheet.create({
  buttonText:{ marginLeft: 5, fontFamily: typography.bold, fontSize: fontSize.font8, color: '#3752FE' },
  container:{ width: 330, padding: 8, paddingRight: 16, backgroundColor: 'white', height: 160, borderRadius: 10 },
  bigText:{fontFamily:typography.medium,fontSize:fontSize.font16,
  },
  smallText:{ fontFamily: typography.medium, fontSize: fontSize.font12, color: color.palette.grey2 },
  buttonStyle: {
    backgroundColor: 'rgba(55, 82, 254, 0.11)',
    borderRadius: 5, paddingVertical: 8, alignSelf: 'flex-start',
