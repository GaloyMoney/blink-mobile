...
const Trophee = (props) => (
  <Svg viewBox="0 0 392.551 392.551" height={70} width={70} {...props}>
...
export const RewardsHeader = ({ isRewardOpen, balance, numTrophee, title, close = () => {} }) => {
  ...
  if (numTrophee === 0) {
    trophees.push(<Trophee fill={palette.lighterGrey} />)
  } else {
    for (let i = 0; i < numTrophee; i++) {
      trophees.push(<Trophee fill={color.primary} />)
    }
  }
  ...
  return (
    <>
      <Animated.View style={styles.header}>
        {!isRewardOpen && <View style={styles.graphic}>{trophees}</View>}
