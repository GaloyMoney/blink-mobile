...
import { ConvertPaymentAmount, GetFeeParams, PaymentDetail, SendPaymentParams } from "@app/screens/send-bitcoin-screen/payment-details"
...
type CreateFunctionWithSpy = <T extends (...args: any) => any>() => ((params: CreateFunctionWithSpyParams<T>) => void)
...
export const expectCannotSendPayment = (paymentDetails: PaymentDetail<WalletCurrency>) => {
...
export const getTestSetSendingWalletDescriptor: CreateFunctionWithSpy = () => ((params) => {
    const { defaultParams, creatorFunction, spy } = params
    const paymentDetails = creatorFunction(defaultParams)
